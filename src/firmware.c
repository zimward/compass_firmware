...
inline void initMAG()
{
    ...
    sendCmd(addr, MAG_CTRL_REG1,
            (1 << OS0) | (1 << AC)); // Set Sample Freq to 40 Hz with 32x OSR;
                                     // Set chip into active mode
    // Offsets
    int16_t x;
    int16_t y;
    int16_t z;
...
inline void init()
{
    ...
    // PORTB |= (1<<PB4);  //enable pullup (setting output bit but leaving it as
    // input)

    /* Setup PWM modulators */
    //       PWM1A EN | Connect Pins to PWM   | Set Freq. Devider to 1/4 (265kHz
    //       eg. 1kHz PWM out)
...
int main(void)
{
    ...
    /*  while(1){
        ...
        float heading = atan2(mag_field[0],mag_field[1]); // atan(x,y) -> north
      clockwise uint16_t degr = (heading > 0 ? heading : (2*PI + heading)) * 360
      / (2*PI); degr %= 360; byte electrode = getElectrode(degr);
